  ...
  <div>
    ...
    <div class="bg-white rounded-xl border border-gray-100 shadow-sm overflow-hidden">
      ...
      <div v-else-if="filteredOrders.length === 0" class="py-16">
        <ui-app-empty-state icon="heroicons:clipboard-document-list" title="Tidak ada pesanan" description="Coba ubah filter atau tambah pesanan baru" />
      </div>
    ...
    <!-- Add Order Modal -->
    <ui-app-modal :show="showAddModal" title="Tambah Pesanan Baru" size="lg" @close="showAddModal = false">
      <form @submit.prevent="submitNewOrder" class="p-6 space-y-4">
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div>
            <label class="block text-sm font-medium text-gray-700 mb-1">Nama Pelanggan *</label>
            <input v-model="form.customerName" type="text" required class="w-full border border-gray-200 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-primary-400" />
          </div>
          <div>
            <label class="block text-sm font-medium text-gray-700 mb-1">No. HP</label>
            <input v-model="form.customerPhone" type="text" class="w-full border border-gray-200 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-primary-400" />
          </div>
          <div>
            <label class="block text-sm font-medium text-gray-700 mb-1">Jenis Pakaian *</label>
            <input v-model="form.garmentType" type="text" required placeholder="Cth: Kemeja Batik" class="w-full border border-gray-200 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-primary-400" />
          </div>
          <div>
            <label class="block text-sm font-medium text-gray-700 mb-1">Deadline *</label>
            <input v-model="form.deadline" type="date" required class="w-full border border-gray-200 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-primary-400" />
          </div>
          <div>
            <label class="block text-sm font-medium text-gray-700 mb-1">Total Biaya (Rp)</label>
            <input v-model="form.totalPrice" type="number" min="0" class="w-full border border-gray-200 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-primary-400" />
          </div>
          <div>
            <label class="block text-sm font-medium text-gray-700 mb-1">DP / Uang Muka (Rp)</label>
            <input v-model="form.paidAmount" type="number" min="0" class="w-full border border-gray-200 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-primary-400" />
          </div>
        </div>
        <div>
          <label class="block text-sm font-medium text-gray-700 mb-1">Deskripsi / Catatan</label>
          <textarea v-model="form.description" rows="3" class="w-full border border-gray-200 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-primary-400 resize-none" placeholder="Warna, bahan, detail ukuran..." />
        </div>
...
const filteredOrders = computed(() => {
  return (orders.value ?? []).filter(o => {
    const matchSearch = !search.value ||
      ...
      o.receiptNumber.toLowerCase().includes(search.value.toLowerCase())
