<template>
  <div>
    ...
    <div class="flex flex-col sm:flex-row gap-3 mb-6 justify-between items-center">
      ...
      <button @click="openAddModal" class="bg-primary-600 hover:bg-primary-700 text-white px-4 py-2.5 rounded-xl text-sm font-medium transition-colors flex items-center gap-2 whitespace-nowrap">
    ...
    <div v-else class="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
      <div v-if="filteredEmployees.length === 0" class="p-8">
        <ui-app-empty-state icon="heroicons:users" title="Tidak ada karyawan" description="Coba ubah filter atau tambah karyawan baru" />
      </div>
        ...
        <table class="w-full text-left border-collapse">
          <thead>
            <tr class="bg-gray-50 border-b border-gray-100 text-sm text-gray-500">
              ...
              <th class="py-3 px-4 font-medium">Tanggal Bergabung</th>
              <th class="py-3 px-4 font-medium text-right">Aksi</th>
            </tr>
          </thead>
          <tbody class="divide-y divide-gray-50 text-sm text-gray-700">
            <tr v-for="emp in filteredEmployees" :key="emp.id" class="hover:bg-gray-50/50 transition-colors group">
              <td class="py-3 px-4">
              ...
              <td class="py-3 px-4 text-right">
                <div class="flex items-center justify-end gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                  <NuxtLink :to="`/admin/employees/${emp.id}`" class="p-1.5 text-gray-400 hover:text-primary-600 rounded-lg hover:bg-primary-50 transition-colors" title="Detail">
                    <Icon name="heroicons:eye" class="w-4 h-4" />
                  </NuxtLink>
                  <button @click="openEditModal(emp)" class="p-1.5 text-gray-400 hover:text-amber-600 rounded-lg hover:bg-amber-50 transition-colors" title="Edit">
                    <Icon name="heroicons:pencil-square" class="w-4 h-4" />
                  </button>
                  <button @click="handleDelete(emp.id)" class="p-1.5 text-gray-400 hover:text-red-600 rounded-lg hover:bg-red-50 transition-colors" title="Hapus">
      ...
      <div class="bg-white rounded-2xl shadow-xl w-full max-w-md overflow-hidden">
        ...
        <form @submit.prevent="saveEmployee" class="p-6 space-y-4">
          <div>
            ...
            <input v-model="formData.name" type="text" required class="w-full px-3 py-2 border border-gray-300 rounded-xl focus:outline-none focus:ring-2 focus:ring-primary-500 focus:border-primary-500" placeholder="Masukkan nama..." />
          </div>
          <div>
            <label class="block text-sm font-medium text-gray-700 mb-1">Role</label>
            <select v-model="formData.role" required class="w-full px-3 py-2 border border-gray-300 rounded-xl focus:outline-none focus:ring-2 focus:ring-primary-500 focus:border-primary-500">
              <option value="Potong">Potong</option>
              <option value="Jahit">Jahit</option>
              <option value="Finishing">Finishing</option>
              <option value="Magang">Magang</option>
          ...
          <div v-if="isEditing">
            ...
            <select v-model="formData.status" required class="w-full px-3 py-2 border border-gray-300 rounded-xl focus:outline-none focus:ring-2 focus:ring-primary-500 focus:border-primary-500">
              <option value="Idle">Idle</option>
              <option value="Working">Working</option>
            </select>
          </div>
          <div class="pt-4 flex gap-3 justify-end">
            <button type="button" @click="closeModal" class="px-4 py-2 text-sm font-medium text-gray-700 bg-white border border-gray-300 rounded-xl hover:bg-gray-50">Batal</button>
            <button type="submit" :disabled="isSaving" class="px-4 py-2 text-sm font-medium text-white bg-primary-600 rounded-xl hover:bg-primary-700 disabled:opacity-50 flex items-center gap-2">
...
const openAddModal = () => {
  ...
  formData.value = { name: '', role: 'Jahit', status: 'Idle' }
...
const openEditModal = (emp: any) => {
  ...
  formData.value = { name: emp.name, role: emp.role, status: emp.status }
  isModalOpen.value = true
}
