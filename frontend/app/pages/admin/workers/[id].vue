    ...
    <template v-else-if="employee">
      ...
      <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div class="bg-white rounded-2xl border border-gray-100 shadow-sm p-6">
            ...
            <template v-else>
              <div class="space-y-2 text-sm">
                <div class="flex justify-between"><span class="text-gray-500">Periode</span><span
                    class="font-medium text-xs text-gray-600">{{ wages?.period }}</span></div>
                <div class="flex justify-between"><span class="text-gray-500">Unit Selesai</span><span
                    class="font-medium">{{ wages?.total_finished }} pc</span></div>
                <hr class="border-gray-100 my-2" />
                <div class="flex justify-between"><span class="text-gray-600 font-medium">Total Upah</span><span
                    class="font-bold text-emerald-600 text-base">{{ formatCurrency(wages?.wage ?? 0) }}</span></div>
              </div>
            </template>
          </div>
        </div>
        ...
        <div class="lg:col-span-2 space-y-4">
          ...
          <div class="bg-white rounded-2xl border border-gray-100 shadow-sm p-6">
            ...
            <template v-if="perfStatus === 'pending'">
              <div class="animate-pulse flex items-end gap-3 h-32">
                <div v-for="i in 6" :key="i" class="flex-1 bg-gray-200 rounded-t"
          ...
          <!-- Performance Summary -->
          <div class="bg-white rounded-2xl border border-gray-100 shadow-sm p-6">
            <h3 class="font-semibold text-gray-900 mb-4">Ringkasan Performa</h3>
            <template v-if="perfStatus === 'pending'">
              <div class="animate-pulse grid grid-cols-2 gap-3">
                <div class="h-16 bg-gray-100 rounded-xl" />
                <div class="h-16 bg-gray-100 rounded-xl" />
              </div>
            </template>
            <template v-else>
              <div class="grid grid-cols-2 gap-3">
                ...
                <div class="bg-primary-50 rounded-xl p-4 text-center">
                  <p class="text-xs text-gray-400 mb-1">Skor Performa</p>
